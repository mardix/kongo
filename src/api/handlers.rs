...
pub async fn meta_operations(
  ...
  "operations":{{
    ...
    "system_refresh_inventory":{{"writes":true,"required":[],"optional":[],"description":"Reinspect discoverable local/S3 DBs, update the internal system catalog inventory, and remove unavailable database rows"}},
    ...
    "insert":{{"writes":true,"required":["namespace","data(object|array<object>)"],"optional":["_user_id","metadata(hidden document metadata)","ttl_seconds","expiry_behavior(archive|delete)","lifecycle(single insert only)","allow_system_timestamps","unique_fields(composite)","on_conflict(skip|error)","commit","dry_run"],"description":"Insert one or many documents. A singular insert may atomically schedule named lifecycle transitions. _user_id can be supplied in payload or per data object and is stored as a document column. metadata is stored separately in _metadata and is hidden from normal reads. commit=false returns prepared pending items with generated ids/timestamps before the queued write commits"}},
    ...
    "update":{{"writes":true,"required":["data(object with _id)|filter+data(object)|data(array<object with _id>)"],"optional":["namespace|scope=all","user_id(explicit single id only)","metadata(explicit single id only)","allow_system_timestamps","replace(single only)","lifecycle(explicit single id only)","max_docs","commit","dry_run"],"description":"Update one or many documents. A singular explicit-id update may reassign its _user_id through payload.user_id, replace its separate _metadata object, and atomically add or replace named lifecycle transitions. data._user_id and data._metadata are rejected; data._namespace is discarded. _created_at/_modified_at require allow_system_timestamps=true. replace=true is only allowed for single-document update by _id. commit=false explicit-id updates return prepared pending items; filter updates return queued acknowledgement"}},
    ...
    "get_namespace_stats":{{"writes":false,"required":["namespace"],"optional":[],"description":"Exact live/archive counts and bytes for one namespace"}},
    "get_data_count":{{"writes":false,"required":[],"optional":[],"description":"Return exact current counts for documents with namespace breakdowns, identity statuses, files, metric events, and all user-created SQL tables"}},
    ...
    "user_create":{{"writes":true,"required":[],"optional":["user_id(non-empty string; generated as dashless uuid4 when omitted)","email","username","phone","first_name","last_name","profile_photo","status(default=active)","status_reason","password_hash","password_algo","requires_password_change(default=false)","provider","provider_user_id","data"],"description":"Create identity user metadata. Does not authenticate; password_hash/token values are app-provided"}},
