...
pub async fn meta_operations(
  ...
  "operations":{{
    ...
    "update":{{"writes":true,"required":["data(object with _id)|filter+data(object)|data(array<object with _id>)"],"optional":["namespace|scope=all","metadata(explicit single id only)","replace(single only)","lifecycle(explicit single id only)","max_docs","commit","dry_run"],"description":"Update one or many documents. A singular explicit-id update may atomically add or replace named lifecycle transitions and replace its separate _metadata object. replace=true is only allowed for single-document update by _id. commit=false explicit-id updates return prepared pending items; filter updates return queued acknowledgement"}},
