...
for _ in $(seq 1 40); do
  if curl -sS -o /dev/null "$GATEWAY_URL" -X POST -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","namespace":"users","payload":{}}' 2>/dev/null; then
    break
  fi
  sleep 0.25
done
...
echo "[4/22] insert"
MISSING_COUNT="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"missing/path","operation":"count","namespace":"users","payload":{}}')"
...
INSERT_RES="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"users","payload":{"ttl_seconds":2,"data":{"name":"Ada","age":34,"score":10}}}')"
assert_contains "$INSERT_RES" '"status":"success"' "insert should succeed"
echo "$INSERT_RES"

INSERT_TS_BLOCKED="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"users","payload":{"data":{"name":"TSBlocked","_created_at":"2024-01-01T00:00:00Z"}}}')"
...
COUNT_RES="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","namespace":"users","payload":{"filter":{"age":{"$gte":18}}}}')"
QUERY_RES="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"query","namespace":"users","payload":{"filter":{"age":{"$gte":18}},"compute":{"full_name":{"$join":["$name"]}}}}')"

echo "$COUNT_RES"
echo "$QUERY_RES"

PROJ_RES="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"query","namespace":"users","payload":{"fields":["name","age"],"exclude_fields":["age","_id"]}}')"
...
SORT_RES="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"sort_users","payload":{"data":[{"name":"Alice","profile":{"age":30}},{"name":"Bob","profile":{"age":40}}]}}')"
assert_contains "$SORT_RES" '"inserted_count":2' "sort seed insert should succeed"
SORT_Q="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"query","namespace":"sort_users","payload":{"sort":{"profile.age":-1}}}')"
...
SORT_STR_Q="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"query","namespace":"sort_users","payload":{"sort":"name"}}')"
...
ARRAY_ELEM_Q="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"query","namespace":"array_paths","payload":{"filter":{"people":{"$elemMatch":{"name":"Ada","age":{"$gte":18}}}}}}')"
assert_not_contains "$ARRAY_ELEM_Q" '"_id":"array-1"' "elemMatch should require the same array element"
NESTED_ARRAY_PATH_Q="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"query","namespace":"array_paths","payload":{"filter":{"departments[].teams[].members[].name":"Ada"}}}')"
assert_contains "$NESTED_ARRAY_PATH_Q" '"_id":"array-1"' "nested array paths should traverse every array level"
POSITIONAL_UPDATE="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"update","namespace":"array_paths","payload":{"commit":true,"data":{"_id":"array-1","shipments.$[shipment].items.$[item].qty":{"$inc":1}},"array_filters":{"shipment":{"status":"processing","warehouse.region":"us-east"},"item":{"qty":{"$gte":2},"tags":{"$includes":"priority"}}}}}')"
assert_contains "$POSITIONAL_UPDATE" '"qty":3' "positional array update should mutate matching nested element"
assert_not_contains "$POSITIONAL_UPDATE" '"qty":2' "positional array update should not return the stale matching quantity"
RENAME_Q="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"update","namespace":"array_paths","payload":{"commit":true,"data":{"_id":"array-1","profile.legacy_name":{"$rename":"profile.display_name"}}}}')"
assert_contains "$RENAME_Q" '"display_name":"Ada Legacy"' "rename should move and overwrite the destination"
assert_not_contains "$RENAME_Q" '"legacy_name"' "rename should remove the source path"

LOOKUP_SEED_BOOKS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"books","payload":{"data":[{"_id":"b1","name":"Book One","vendor_id":"v1"},{"_id":"b2","name":"Book Two","vendor_id":"v2"}]}}')"
assert_contains "$LOOKUP_SEED_BOOKS" '"inserted_count":2' "lookup books seed should succeed"
LOOKUP_SEED_VENDORS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"vendors_lkp","payload":{"data":[{"_id":"v1","name":"Vendor One"},{"_id":"v2","name":"Vendor Two"}]}}')"
assert_contains "$LOOKUP_SEED_VENDORS" '"inserted_count":2' "lookup vendors seed should succeed"
LOOKUP_SEED_USERS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"users_lookup","payload":{"data":{"name":"Reader","favorite_books":["b2","b1"]}}}')"
assert_contains "$LOOKUP_SEED_USERS" '"status":"success"' "lookup user seed should succeed"
LOOKUP_Q="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"query","namespace":"users_lookup","payload":{"lookups":{"books":{"from":"books","local_field":"favorite_books[]","foreign_field":"_id","match":"$in","multi":true,"preserve_order":true,"limit":10,"fields":["_id","name","vendor_id"]},"vendors":{"from":"vendors_lkp","local_field":"$lookup.books[].vendor_id","foreign_field":"_id","match":"$in","multi":true,"limit":10}}}}')"
...
SQL_READ="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"sql_execute","payload":{"sql":"SELECT collection AS namespace, COUNT(*) AS total FROM __kdb_documents WHERE collection = ? GROUP BY collection","params":["users"]}}')"
assert_contains "$SQL_READ" '"status":"success"' "sql_execute select should succeed"
assert_contains "$SQL_READ" '"namespace":"users"' "sql_execute should return selected rows"
...
EXPORT_RES="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d "{\"db\":\"smoke\",\"operation\":\"export_jsonl\",\"namespace\":\"users\",\"payload\":{\"target_path\":\"$EXPORT_PATH\"}}")"
...
EOF
IMPORT_RES="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d "{\"db\":\"smoke\",\"operation\":\"import_jsonl\",\"namespace\":\"users_import\",\"payload\":{\"source_path\":\"$IMPORT_PATH\",\"on_conflict\":\"error\"}}")"
...
sleep 3
LIVE_AFTER="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","namespace":"users","payload":{}}')"
ARCH_AFTER="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","namespace":"users","payload":{"archive_only":true}}')"
...
curl -sS -D - -o /dev/null -X POST "$GATEWAY_URL" \
  ...
  -d '{"db":"smoke","operation":"count","namespace":"users","payload":{}}' \
  | grep -Ei 'content-encoding|vary|content-type' || true

echo "[7/22] update data(array<object with _id>)"
INS_BULK="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"users","payload":{"data":[{"name":"U1","status":"new"},{"name":"U2","status":"new"}]}}')"
...
UPD_IDS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d "{\"db\":\"smoke\",\"operation\":\"update\",\"namespace\":\"users\",\"payload\":{\"data\":[{\"_id\":\"$ID1\",\"status\":\"active\"},{\"_id\":\"$ID2\",\"status\":\"active\"}]}}")"
assert_contains "$UPD_IDS" '"updated_count":2' "update data[] should update 2 rows"

echo "[8/22] update filter + data(object)"
UPD_FILTER="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"update","namespace":"users","payload":{"filter":{"status":{"$eq":"active"}},"data":{"tier":"pro"},"max_docs":1}}')"
assert_contains "$UPD_FILTER" '"updated_count":1' "update by filter should respect max_docs"

echo "[9/22] update replace=true single"
UPD_ARRAY="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d "{\"db\":\"smoke\",\"operation\":\"update\",\"namespace\":\"users\",\"payload\":{\"replace\":true,\"data\":{\"_id\":\"$ID1\",\"name\":\"Replaced\",\"only\":\"this\"}}}")"
...
UPSERT_BAD_ID="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"upsert","namespace":"users","payload":{"filter":{"email":{"$eq":"x@example.com"}},"insert_data":{"_id":"bad","email":"x@example.com"},"update_data":{"name":"X"}}}')"
...
UPSERT_EMPTY_FILTER="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"upsert","namespace":"users","payload":{"filter":{},"insert_data":{"email":"z@example.com"},"update_data":{"name":"Z"}}}')"
assert_contains "$UPSERT_EMPTY_FILTER" '"status":"error"' "upsert should reject empty filter"

UPSERT_EXACT_ID="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"upsert","namespace":"users","payload":{"filter":{"_id":{"$eq":"upsert-fixed-id"}},"insert_data":{"name":"Fixed"},"update_data":{"name":"Updated"}}}')"
assert_contains "$UPSERT_EXACT_ID" '"_id":"upsert-fixed-id"' "upsert exact _id filter should preserve the insertion id"

OLD_GET="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"get","namespace":"users","payload":{"id":"upsert-fixed-id"}}')"
assert_contains "$OLD_GET" 'unknown operation: get' "removed get operation should not remain as an alias"

OLD_SET="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"set","namespace":"users","payload":{"data":{"_id":"upsert-fixed-id","name":"Nope"}}}')"
assert_contains "$OLD_SET" 'unknown operation: set' "removed set operation should not remain as an alias"

OLD_SEARCH="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"search","namespace":"users","payload":{"search":"Fixed"}}')"
assert_contains "$OLD_SEARCH" 'unknown operation: search' "removed search operation should not remain as an alias"

DEL_CONFLICT="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d "{\"db\":\"smoke\",\"operation\":\"delete\",\"namespace\":\"users\",\"payload\":{\"id\":\"$ID1\",\"ids\":[\"$ID2\"]}}")"
assert_contains "$DEL_CONFLICT" '"status":"error"' "delete should reject id+ids together"

echo "[11/22] insert composite unique_fields"
INS_ABS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"users","payload":{"data":{"email":"nobody@example.com","tenant":{"id":"t1"},"name":"Nobody"},"unique_fields":["tenant.id","email"],"on_conflict":"skip"}}')"
assert_contains "$INS_ABS" '"inserted_count":1' "insert should insert first composite unique row"
INS_ABS2="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"users","payload":{"data":{"email":"nobody@example.com","tenant":{"id":"t1"},"name":"Nobody Again"},"unique_fields":["tenant.id","email"],"on_conflict":"skip"}}')"
assert_contains "$INS_ABS2" '"skipped_count":1' "insert should skip composite unique conflict"

TX_UNIQUE_SKIP="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"transaction","payload":{"operations":[{"operation":"insert","namespace":"tx_unique","payload":{"data":{"tenant":{"id":"t1"},"email":"same@example.com","name":"First"},"unique_fields":["tenant.id","email"],"on_conflict":"skip"}},{"operation":"insert","namespace":"tx_unique","payload":{"data":{"tenant":{"id":"t1"},"email":"same@example.com","name":"Second"},"unique_fields":["tenant.id","email"],"on_conflict":"skip"}}]}}')"
...
TX_UNIQUE_ERROR="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"transaction","payload":{"operations":[{"operation":"insert","namespace":"tx_unique","payload":{"data":{"tenant":{"id":"t2"},"email":"rollback@example.com","name":"Rollback First"},"unique_fields":["tenant.id","email"]}},{"operation":"insert","namespace":"tx_unique","payload":{"data":{"tenant":{"id":"t2"},"email":"rollback@example.com","name":"Rollback Second"},"unique_fields":["tenant.id","email"]}}]}}')"
...
TX_CONTINUE="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"transaction","payload":{"on_error":"continue","operations":[{"alias":"first","operation":"insert","namespace":"tx_continue","payload":{"data":{"_id":"tx-continue-1","name":"First"}}},{"alias":"duplicate-id","operation":"insert","namespace":"tx_continue","payload":{"data":{"_id":"tx-continue-1","name":"Duplicate"}}},{"alias":"broken","operation":"update","namespace":"tx_continue","payload":{"data":{"name":"Missing id"}}},{"alias":"last","operation":"insert","namespace":"tx_continue","payload":{"data":{"_id":"tx-continue-2","name":"Last"}}}]}}')"
assert_contains "$TX_CONTINUE" '"status":"partial"' "transaction continue mode should report partial status"
assert_contains "$TX_CONTINUE" '"succeeded":2' "transaction continue mode should commit successful children"
assert_contains "$TX_CONTINUE" '"failed":2' "transaction continue mode should report SQL and validation failures"
assert_contains "$TX_CONTINUE" '"alias":"duplicate-id"' "transaction should recover from a SQLite constraint failure"
assert_contains "$TX_CONTINUE" '"alias":"broken"' "transaction results should retain optional aliases"
TX_CONTINUE_COUNT="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","namespace":"tx_continue","payload":{}}')"
assert_contains "$TX_CONTINUE_COUNT" '"count":2' "transaction continue mode should persist both successful children"
TX_OLD_TOP_LEVEL_DATA="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"transaction","data":[{"operation":"insert","namespace":"tx_unique","payload":{"data":{"name":"legacy-shape"}}}]}')"
assert_contains "$TX_OLD_TOP_LEVEL_DATA" '"status":"error"' "transaction should reject removed top-level data shape"
assert_contains "$TX_OLD_TOP_LEVEL_DATA" 'payload.operations' "transaction old shape error should identify payload.operations"
TX_EMPTY_OPERATIONS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"transaction","payload":{"operations":[]}}')"
assert_contains "$TX_EMPTY_OPERATIONS" '"status":"error"' "transaction should reject an empty operations array"
assert_contains "$TX_EMPTY_OPERATIONS" 'cannot be empty' "transaction empty operations error should be explicit"
TX_UPSERT="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"transaction","payload":{"operations":[{"alias":"create","operation":"upsert","namespace":"tx_upsert","payload":{"filter":{"email":"ada@example.com"},"insert_data":{"email":"ada@example.com","visits":1},"update_data":{"visits":{"$inc":1}}}},{"alias":"increment","operation":"upsert","namespace":"tx_upsert","payload":{"filter":{"email":"ada@example.com"},"insert_data":{"email":"ada@example.com","visits":1},"update_data":{"visits":{"$inc":1}}}},{"alias":"no-change","operation":"upsert","namespace":"tx_upsert","payload":{"filter":{"email":"ada@example.com"},"insert_data":{"email":"ada@example.com","visits":1},"max_docs":0}},{"alias":"insert-only","operation":"upsert","namespace":"tx_upsert","payload":{"filter":{"email":"grace@example.com"},"insert_data":{"email":"grace@example.com","visits":1},"max_docs":0}}]}}')"
assert_contains "$TX_UPSERT" '"status":"success"' "transaction upserts should commit"
assert_contains "$TX_UPSERT" '"action":"inserted"' "transaction upsert should report its insert path"
assert_contains "$TX_UPSERT" '"action":"updated"' "later transaction upsert should see and update the earlier insert"
assert_contains "$TX_UPSERT" '"action":"no_change"' "transaction upsert max_docs=0 should report no change for a match"
assert_contains "$TX_UPSERT" '"alias":"increment"' "transaction upsert result should retain its alias"
assert_contains "$TX_UPSERT" '"visits":2' "transaction upsert should apply mutation operators"
TX_UPSERT_COUNT="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","namespace":"tx_upsert","payload":{}}')"
assert_contains "$TX_UPSERT_COUNT" '"count":2' "transaction upsert should persist insert and update paths"
TX_UPSERT_ROLLBACK="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"transaction","payload":{"operations":[{"operation":"upsert","namespace":"tx_upsert","payload":{"filter":{"_id":"tx-upsert-rollback"},"insert_data":{"email":"first@example.com"},"update_data":{"email":"first@example.com"}}},{"operation":"upsert","namespace":"tx_upsert","payload":{"filter":{"_id":"tx-upsert-rollback","email":"different@example.com"},"insert_data":{"email":"different@example.com"},"update_data":{"email":"different@example.com"}}}]}}')"
assert_contains "$TX_UPSERT_ROLLBACK" '"status":"error"' "transaction upsert conflict should fail in default fail mode"
TX_UPSERT_ROLLBACK_COUNT="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","namespace":"tx_upsert","payload":{"filter":{"_id":"tx-upsert-rollback"}}}')"
assert_contains "$TX_UPSERT_ROLLBACK_COUNT" '"count":0' "transaction upsert conflict should roll back earlier children"
...
SET_ONE="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d "{\"db\":\"smoke\",\"operation\":\"update\",\"payload\":{\"data\":{\"_id\":\"$ID1\",\"flag\":\"ok\"}}}")"
assert_contains "$SET_ONE" '"updated_count":1' "update should update row by id"

echo "[13/22] set_ttl + delete soft-delete"
SET_TTL="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d "{\"db\":\"smoke\",\"operation\":\"set_ttl\",\"namespace\":\"users\",\"payload\":{\"ids\":[\"$ID1\"],\"ttl_seconds\":1}}")"
assert_contains "$SET_TTL" '"updated_count":1' "set_ttl should update ttl"
sleep 2
REAP_AFTER_TTL="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","namespace":"users","payload":{"archive_only":true}}')"
assert_contains "$REAP_AFTER_TTL" '"count":' "archive count after set_ttl should be returned"
ARCH_BULK="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d "{\"db\":\"smoke\",\"operation\":\"delete\",\"namespace\":\"users\",\"payload\":{\"ids\":[\"$ID2\"]}}")"
assert_contains "$ARCH_BULK" '"soft_delete":true' "delete should soft-delete one row by default"

echo "[14/22] scope=all count without namespace"
SCOPE_ALL="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","payload":{"scope":"all"}}')"
assert_contains "$SCOPE_ALL" '"status":"success"' "scope all count should work without namespace"

echo "[15/22] get_namespace_stats + recompute_stats + list_namespaces"
INS_PROJECTS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"projects","payload":{"data":[{"name":"P1"},{"name":"P2"}]}}')"
assert_contains "$INS_PROJECTS" '"inserted_count":2' "projects insert should succeed"
STATS_PROJECTS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"get_namespace_stats","namespace":"projects","payload":{}}')"
assert_contains "$STATS_PROJECTS" '"live_count":2' "stats should show 2 live docs"
RECOMP_STATS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"recompute_stats","namespace":"projects","payload":{}}')"
...
assert_contains "$LIST_COLLS" '"namespace":"projects"' "list_namespaces should include projects"
LIST_TABLES="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"sql_list_tables","payload":{}}')"
assert_contains "$LIST_TABLES" '"status":"success"' "sql_list_tables should succeed"
assert_not_contains "$LIST_TABLES" '__kdb_' "sql_list_tables should exclude internal tables"
...
LIST_TABLES_AFTER_DDL="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"sql_list_tables","payload":{}}')"
assert_contains "$LIST_TABLES_AFTER_DDL" '"name":"app_notes"' "sql_list_tables should include user-created tables"
DATA_COUNT="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"get_data_count","payload":{}}')"
assert_contains "$DATA_COUNT" '"documents":' "get_data_count should include document totals"
assert_contains "$DATA_COUNT" '"namespace":"projects"' "get_data_count should include namespace breakdowns"
assert_contains "$DATA_COUNT" '"users":' "get_data_count should include identity totals"
assert_contains "$DATA_COUNT" '"files":' "get_data_count should include file totals"
assert_contains "$DATA_COUNT" '"metrics":' "get_data_count should include metric event totals"
assert_contains "$DATA_COUNT" '"name":"app_notes"' "get_data_count should include user-created tables"
DATA_COUNT_OPTIONS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"get_data_count","payload":{"limit":1}}')"
assert_contains "$DATA_COUNT_OPTIONS" '"status":"error"' "get_data_count should reject payload options"
...
STATS_SRC_AFTER_MOVE="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"get_namespace_stats","namespace":"projects","payload":{}}')"
assert_contains "$STATS_SRC_AFTER_MOVE" '"live_count":1' "source namespace stats should decrement after change_namespace"
STATS_DST_AFTER_MOVE="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"get_namespace_stats","namespace":"projects___kdb_archived","payload":{}}')"
...
INS_RENAME="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"ns_old","payload":{"data":[{"r":1},{"r":2}]}}')"
...
COUNT_NEW="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"count","namespace":"ns_new","payload":{}}')"
...
DROP_COL="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"drop_namespace","namespace":"projects","payload":{"ttl_seconds":30}}')"
...
INS_TMP="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"tmpdrop","payload":{"data":[{"a":1},{"a":2}]}}')"
assert_contains "$INS_TMP" '"inserted_count":2' "tmpdrop insert should succeed"
DROP_HARD="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"drop_namespace","namespace":"tmpdrop","payload":{"purge":true}}')"
...
INS_PURGE_IDS="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"purgeids","payload":{"data":[{"x":1},{"x":2}]}}')"
...
echo "[22/22] restore_archive(namespace/filter) conflict-skip"
INS_RESTORE="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","namespace":"restorec","payload":{"data":[{"z":1},{"z":2}]}}')"
...
INS_CONFLICT="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d "{\"db\":\"smoke\",\"operation\":\"insert\",\"namespace\":\"restorec\",\"payload\":{\"data\":{\"_id\":\"$RID1\",\"z\":100}}}")"
assert_contains "$INS_CONFLICT" '"status":"success"' "conflict seed insert should succeed"
RESTORE_FILTER="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"restore_archive","namespace":"restorec","payload":{}}')"
...
echo "curl -sS -X POST ${GATEWAY_URL} -H 'content-type: application/json' -d '{\"db\":\"myapp.something/main\",\"operation\":\"count\",\"namespace\":\"users\",\"payload\":{}}'"
echo "curl -sS -X POST ${GATEWAY_URL} -H 'content-type: application/json' -d '{\"db\":\"myapp.something/main\",\"operation\":\"query\",\"namespace\":\"users\",\"payload\":{\"filter\":{\"age\":{\"\$gte\":18}},\"compute\":{\"full_name\":{\"\$join\":[\"\$first_name\",\" \",\"\$last_name\"]}}}}'"
