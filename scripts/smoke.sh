...
MACRO_INSERT="$(curl -sS -X POST "$GATEWAY_URL" -H 'content-type: application/json' -d '{"db":"smoke","operation":"insert","payload":{"collection":"users","data":{"_id":{"$id_uuidv4":{"prefix":"sessions:"}},"ts":{"$ts_now":true},"sid":{"$id_random":{"len":10}}}}}')"
